...
Supports `--mode headless` and `--mode gui` (the `gui` mode is referenced in constants but **not yet implemented**).
...
- No standalone desktop or web application — must know Python
- No file picker — RDSR file path must be typed as a string
- No form-based settings editor — settings require editing JSON or Python code
- No way to compare two procedures or two phantom configurations side by side
- No summary report export (PDF, Word) — only raw dict/JSON/HTML
- No progress feedback beyond a tqdm bar in the terminal
- The `gui` mode constant exists in the code but is never implemented
